...
import type { CalculatorState, CostTip, TipActionField } from "@/types/calculator";
import { calculateCosts, getCostReductionTips } from "@/utils/calculate";
...
export function ToolPageContent() {
  ...
  const metrics = useMemo(() => calculateCosts(state), [state]);
  const costTips = useMemo(() => getCostReductionTips(state, metrics), [state, metrics]);
  ...
  const updateState = (next: Partial<CalculatorState>) => {
  ...
  const applyTip = (tip: CostTip) => {
    ...
    const currentMonthly = metrics.totalMonthlyCost;
    const nextMonthly = calculateCosts(nextState).totalMonthlyCost;
          ...
          {!isToolInterfaceCollapsed ? (
            <div id="tool-interface-content" className="mt-6 space-y-6">
              <div className="grid gap-6 xl:grid-cols-2">
                <ModelSection state={state} onChange={updateState} />
                <HeartbeatSection
                  state={state}
                  heartbeatsPerDay={metrics.heartbeatsPerDay}
                  dailyHeartbeatTokens={metrics.dailyHeartbeatTokens}
                  dailyHeartbeatCost={metrics.dailyHeartbeatCost}
                  monthlyHeartbeatCost={metrics.monthlyHeartbeatCost}
                  showWarning={metrics.showHeartbeatWarning}
                  warningPercent={metrics.heartbeatPctOfTotal}
              ...
              <SummaryCard
                ...
                messageCost={metrics.dailyMessageCost}
                heartbeatCost={metrics.dailyHeartbeatCost}
                costPerThousandMessages={metrics.costPerThousandMessages}
